...
def aktualisiere_Vertriebskacheln(neue_Raster_p, bundesland, massstab, utm, layer, dtk_nummern):
        ...
        for file in files:
            if file.endswith(f'{layer}.tif'):
                ...
                if layer != "col":                      # Pixelwerte der schwarz-weiß-Layer tauschen für Zusammenführung mit Bestandsrastern
                    ...
                    processing.run("gdal:tileindex", {'LAYERS': [pfad],
                                                      'PATH_FIELD_NAME': 'location', 'ABSOLUTE_PATH': False,
                                                      'PROJ_DIFFERENCE': False, 'TARGET_CRS': None,
                                                      'CRS_FIELD_NAME': '', 'CRS_FORMAT': 0,
                                                      'OUTPUT': r"%s/Zwischenschritte/Vektor/polygon_%s.gpkg"%(proj, dtk_nr)})
                poly_pfade.append(r"%s/Zwischenschritte/Vektor/polygon_%s.gpkg"%(proj, dtk_nr))
    ...
    if QgsProject.instance().mapLayersByName(f'r_clip2x_{layer}_'):
        ...
        if layer != "col": #or raster_clip_kb[0][0] == "r":
            clip_param['NODATA'] = 0
